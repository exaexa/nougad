...
double
compare_floats_relative(F a, F b)
{
...
private:
  ...
  std::vector<real_t>
    mPoints; ///< Container storing the features (row-wise, padded to alignment)
  real_t *mDataPtr;
...
public:
  DataPoints(std::size_t dim = 0, std::size_t size = 0, F *data = nullptr)
    : mDim(dim)
    , mSize(size)
    , mDataPtr(data)
  {
