...
template <typename F>
__inline__ __device__ void computeResiduals(const F* const __restrict__ points, const F* const __restrict__ spectra,
											const F* const __restrict__ result, F* const __restrict__ residuals, const std::uint32_t dim,
...
__global__ void nougadBaseKernel(const F* __restrict__ points, const F* const __restrict__ spectra,
									 ...
									 const std::uint32_t iterations, const F alpha, const F acceleration)
{
	...
	for (std::uint32_t i = 0; i < iterations; ++i)
	{
		...
		for (std::uint32_t k_idx = 0; k_idx < spectrumN; ++k_idx)
		{
			...
			for (std::uint32_t d_idx = 0; d_idx < dim; ++d_idx)
			{
				const F w = (resultResiduals[d_idx] > 0 ? spectraPositiveWeights : spectraNegativeWeights)[k_idx * dim + d_idx];
...
__global__ void nougadBaseSharedKernel(const F* __restrict__ points, const F* const __restrict__ spectra,
										   ...
										   const std::uint32_t spectrumN, const std::uint32_t iterations, const F alpha, const F acceleration)
{
	extern __shared__ char sharedMemory[];
	F* const __restrict__ spectraCache = reinterpret_cast<F*>(sharedMemory);
	F* const __restrict__ spectraPositiveWeightsCache = spectraCache + dim * spectrumN;
	F* const __restrict__ spectraNegativeWeightsCache = spectraPositiveWeightsCache + dim * spectrumN;
	F* const __restrict__ resultWeightsCache = spectraNegativeWeightsCache + dim * spectrumN;
	F* const __restrict__ gradientMemoryCache = resultWeightsCache + spectrumN + threadIdx.x * spectrumN;
	F* const __restrict__ resultResidualsCache = resultWeightsCache + spectrumN + blockDim.x * spectrumN + threadIdx.x * dim;
	F* const __restrict__ resultCache = resultWeightsCache + spectrumN + blockDim.x * spectrumN + blockDim.x * dim + threadIdx.x * spectrumN;
	...
	// store to shared memory
	{
		...
		if (threadIdx.x == 0)
		{
			...
			memcpy(spectraPositiveWeightsCache, spectraPositiveWeights, dim * spectrumN * sizeof(F));
			memcpy(spectraNegativeWeightsCache, spectraNegativeWeights, dim * spectrumN * sizeof(F));
	...
	for (std::uint32_t i = 0; i < iterations; ++i)
	{
		computeResiduals(points, spectraCache, resultCache, resultResidualsCache, dim, spectrumN);

		for (std::uint32_t k_idx = 0; k_idx < spectrumN; ++k_idx)
		{
			F gradient = resultCache[k_idx] > 0 ? 0 : resultWeightsCache[k_idx] * resultCache[k_idx];

			for (std::uint32_t d_idx = 0; d_idx < dim; ++d_idx)
			{
				const F w = (resultResidualsCache[d_idx] > 0 ? spectraPositiveWeightsCache : spectraNegativeWeightsCache)[k_idx * dim + d_idx];
				gradient += resultResidualsCache[d_idx] * spectraCache[k_idx * dim + d_idx] * w;
			}
	...
	computeResiduals(points, spectraCache, resultCache, resultResiduals, dim, spectrumN);
