...
public:
	...
	IGradientDescendAlgorithm() : mDim(0), mN(0), mSpectrumN(0), mIterations(0), mAlpha(0), mAcceleration(0), mResultResiduals(0), mResult(0) {}
							...
							DataPoints<F>& resultResiduals, const std::size_t iterations, const F alpha, const F acceleration)
	{
		...
		{
			throw(
				bpp::RuntimeError() << "Result points dimension and size are different than such implied by measurement points and spectra. Result: "
									<< initialResult.size() << "x" << initialResult.getDim() << "Measurements: " << points.size() << "x"
									<< points.getDim() << " Spectra: " << spectra.size() << "x" << spectra.getDim());
		}

		if (spectra.getDim() != mDim)
		{
			throw(bpp::RuntimeError() << "Measurement points and spectra have different dimensions: " << mDim << " != " << spectra.getDim());
		}

		if (spectra.getDim() != spectraNegativeWeights.getDim() || spectra.getDim() != spectraPositiveWeights.getDim()
			|| spectra.size() != spectraNegativeWeights.size() || spectra.size() != spectraPositiveWeights.size())
		{
			throw(bpp::RuntimeError() << "Spectra weights dimension and size are different than spectra. Spectra: " << spectra.size() << "x"
									  << spectra.getDim() << "Positive Weights: " << spectraPositiveWeights.size() << "x"
									  << spectraPositiveWeights.getDim() << " Negative Weights: " << spectraNegativeWeights.size() << "x"
									  << spectraNegativeWeights.getDim());
		}

		if (resultWeights.getDim() != mSpectrumN || resultWeights.size() != 1)
		{
			throw(bpp::RuntimeError() << "Results points and result weights have different dimensions: " << mSpectrumN
	...
	bool verifyResult(IGradientDescendAlgorithm<F>& refAlgorithm, std::ostream& out)
	{
		...
		for (size_t resultI = 0; resultI < 2; resultI++)
		{
			...
			for (std::size_t p = 0; p < mN; ++p)
			{
				...
				{
					if (++errors < 16)
					{ // only first 16 errors are printed out
						out << (resultI == 0? "Result" : "Residual") << " Error [" << p << "]:" << std::endl;
