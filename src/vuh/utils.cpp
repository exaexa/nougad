...
auto
read_spirv(const char *filename) -> std::vector<uint32_t>
{
  auto fin = std::ifstream(filename, std::ios::binary | std::ios::ate);
  if (!fin.is_open()) {
    throw std::runtime_error(std::string("failed opening file ") + filename +
                             " for reading");
  }
...
namespace arr {
/// Copy data between device buffers using the device transfer command pool and
/// queue. Source and destination buffers are supposed to be allocated on the
/// same device. Fully sync, no latency hiding whatsoever.
auto
copyBuf(vuh::Device &device ///< device where buffers are allocated
        ,
