...
namespace {
#ifndef NDEBUG
static const std::array<const char *, 1> default_layers = {
  "VK_LAYER_LUNARG_standard_validation"
};
static const std::array<const char *, 1> default_extensions = {
...
auto
contains(const char *x, const std::vector<U> &array, F &&fun) -> bool
{
  return end(array) != std::find_if(ALL(array), [&](auto &r) {
           return 0 == std::strcmp(x, fun(r));
         });
}

/// Extend vector of string literals by candidate values that have a macth in a
/// reference set.
template<class U, class T, class F>
auto
filter_list(
  std::vector<const char *> old_values ///< array to extend
  ,
  ...
  ,
  const char *layer_msg =
    nullptr ///< base part of the log message about unsuccessful candidate value
...
/// Filter requested layers, throw away those not present on particular
/// instance. Add default validation layers to debug build.
auto
filter_layers(const std::vector<const char *> &layers)
{
  const auto avail_layers = vk::enumerateInstanceLayerProperties();
  auto r = filter_list(
    {}, layers, avail_layers, [](const auto &l) { return l.layerName; });
  r =
    filter_list(std::move(r), default_layers, avail_layers, [](const auto &l) {
      return l.layerName;
    });
  return r;
}

/// Filter requested extensions, throw away those not present on particular
/// instance. Add default debug extensions to debug build.
auto
filter_extensions(const std::vector<const char *> &extensions)
{
  const auto avail_extensions = vk::enumerateInstanceExtensionProperties();
  auto r = filter_list({}, extensions, avail_extensions, [](const auto &l) {
    return l.extensionName;
  });
...
static auto VKAPI_ATTR
debugReporter(VkDebugReportFlagsEXT,
...
auto
createInstance(const std::vector<const char *> layers,
               const std::vector<const char *> extensions,
               const vk::ApplicationInfo &info) -> vk::Instance
{
  auto createInfo = vk::InstanceCreateInfo(
    vk::InstanceCreateFlags(), &info, ARR_VIEW(layers), ARR_VIEW(extensions));
...
auto
registerReporter(vk::Instance instance, vuh::debug_reporter_t reporter)
...
/// In debug build in addition to user-defined layers attempts to load
/// validation layers.
Instance::Instance(const std::vector<const char *> &layers,
  ...
  : _instance(
      createInstance(filter_layers(layers), filter_extensions(extension), info))
...
/// Move assignment
auto
Instance::operator=(Instance &&o) noexcept -> Instance &
{
  using std::swap;
...
auto
Instance::devices() -> std::vector<Device>
{
...
Instance::report(
  const char
    *prefix ///< prefix part of message. may contain component name, etc.
  ,
...
{
  _reporter(
    flags, VkDebugReportObjectTypeEXT{}, 0, 0, 0, prefix, message, nullptr);
}
} // namespace vuh
