...
createDevice(
  ...
  ,
  uint32_t
    compute_family_id ///< index of queue family supporting compute operations
  ,
  uint32_t
    transfer_family_id ///< index of queue family supporting transfer operations
  ) -> vk::Device
{
  ...
  queueCIs[0] = vk::DeviceQueueCreateInfo(
    vk::DeviceQueueCreateFlags(), compute_family_id, 1, &p);
  auto n_queues = uint32_t(1);
  if (transfer_family_id != compute_family_id) {
    queueCIs[1] = vk::DeviceQueueCreateInfo(
      vk::DeviceQueueCreateFlags(), transfer_family_id, 1, &p);
    n_queues += 1;
  }
  auto devCI =
    vk::DeviceCreateInfo(vk::DeviceCreateFlags(), n_queues, queueCIs.data());

  return physicalDevice.createDevice(devCI, nullptr);
}

/// @return preffered family id for the desired queue flags combination, or -1
/// if none is found. If several queues matching required flags combination is
/// available selects the one with minimal numeric value of its flags
/// combination.
auto
getFamilyID(const std::vector<vk::QueueFamilyProperties>
              &queue_families ///< array of queue family properties
            ,
            ...
            ) -> uint32_t
{
  ...
  uint32_t i = 0;
  for (auto &&q : queue_families) { // find the family with a min flag value
                                    // among all acceptable
    const auto flags_i = q.queueFlags;
    if (0 < q.queueCount && (tgtFlag & flags_i) &&
...
auto
allocCmdBuffer(vk::Device device,
               vk::CommandPool pool,
               vk::CommandBufferLevel level = vk::CommandBufferLevel::ePrimary)
  -> vk::CommandBuffer
{
  auto commandBufferAI = vk::CommandBufferAllocateInfo(
    pool, level, 1); // 1 is the command buffer count here
  return device.allocateCommandBuffers(commandBufferAI)[0];
}
} // namespace

namespace vuh {
/// Constructs logical device wrapping the physical device of the given
/// instance.
...
Device::Device(Instance &instance,
  ...
  : Device(instance,
           physdevice,
           getFamilyID(familyProperties, vk::QueueFlagBits::eCompute),
...
{
  try {
    _cmdpool_compute = createCommandPool(
      { vk::CommandPoolCreateFlagBits::eResetCommandBuffer, computeFamilyId });
...
/// Copy constructor. Creates new handle to the same physical device, and
/// recreates associated pools
Device::Device(const Device &other)
  : Device(other._instance,
           other._physdev,
           other._cmp_family_id,
           other._tfr_family_id)
{}

/// Copy assignment. Created new handle to the same physical device and
/// recreates associated pools.
auto
...
/// Move assignment.
auto
Device::operator=(Device &&o) noexcept -> Device &
{
  swap(*this, o);
  return *this;
}
...
auto
Device::properties() const -> vk::PhysicalDeviceProperties
{
...
auto
Device::memoryProperties(uint32_t id) const -> vk::MemoryPropertyFlags
{
...
auto
Device::selectMemory(vk::Buffer buffer,
                     vk::MemoryPropertyFlags properties) const -> uint32_t
{
  ...
  for (uint32_t i = 0; i < memProperties.memoryTypeCount; ++i) {
    if ((memoryReqs.memoryTypeBits & (1u << i)) &&
        ((properties & memProperties.memoryTypes[i].propertyFlags) ==
         properties)) {
      return i;
    }
  }
...
/// @return true if compute queues family is different from that for transfer
/// queues
auto
Device::hasSeparateQueues() const -> bool
{
...
auto
Device::computeQueue(uint32_t i) -> vk::Queue
{
  return getQueue(_cmp_family_id, i);
}

/// Create compute pipeline with a given layout.
/// Shader stage info incapsulates the shader and layout&values of
...
Device::createPipeline(
  ...
  vk::PipelineCreateFlags flags) -> vk::Pipeline
{
  auto pipelineCI =
    vk::ComputePipelineCreateInfo(flags, shader_stage_info, pipe_layout);
  return createComputePipeline(pipe_cache, pipelineCI, nullptr);
}

/// Detach the current compute command buffer for sync operations and create the
/// new one.
/// @return the old buffer handle
auto
Device::releaseComputeCmdBuffer() -> vk::CommandBuffer
{
...
auto
Device::transferQueue(uint32_t i) -> vk::Queue
{
...
auto
Device::alloc(vk::Buffer buf, uint32_t memory_id) -> vk::DeviceMemory
{
...
auto
Device::transferCmdPool() -> vk::CommandPool
{
  return _cmdpool_transfer;
}

/// @return handle to command buffer for syncronous transfer commands
auto
Device::transferCmdBuffer() -> vk::CommandBuffer &
{
