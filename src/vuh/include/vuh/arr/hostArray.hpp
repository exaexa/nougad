...
/// host-visible space. Memory allocation and underlying buffer creation is
/// managed by allocator defined by a template parameter. Such allocator is
/// expected to allocate memory in host-visible GPU memory, mappable for host
/// access. Provides Forward iterator + random-access interface. Memory remains
/// mapped during the whole lifetime of an object of this type.
...
public:
  ...
  HostArray(vuh::Device &device ///< device to create array on
            ...
            ///< additional (to defined by allocator) buffer usage flags
            )
    : BasicArray<Alloc>(device,
                        ...
                        flags_buffer)
    , _data(static_cast<T *>(
        Base::_dev.mapMemory(Base::_mem, 0, n_elements * sizeof(T))))
    , _size(n_elements)
  {}
  ...
  /// Host-accessible iterator to the end (one past the last element) of array
  /// data.
  auto end() -> value_type * { return begin() + size(); }
  auto end() const -> const value_type * { return begin() + size(); }

  ///
  auto device_begin() -> ArrayIter<HostArray>
  {
    return ArrayIter<HostArray>(*this, 0);
  }
  auto device_begin() const -> ArrayIter<HostArray>
  {
    return ArrayIter<HostArray>(*this, 0);
  }
  friend auto device_begin(HostArray &a) -> ArrayIter<HostArray>
  {
    return a.device_begin();
  }

  ///
  auto device_end() -> ArrayIter<HostArray>
  {
    return ArrayIter<HostArray>(*this, _size);
  }
  auto device_end() const -> ArrayIter<HostArray>
  {
    return ArrayIter<HostArray>(*this, _size);
  }
  friend auto device_end(HostArray &a) -> ArrayIter<HostArray>
  {
    return a.device_end();
  }
...
private:        // data
  T *_data;     ///< host accessible pointer to the beginning of corresponding
                ///< memory chunk.
