...
struct _CmdBuffer
{
  /// Constructor. Creates the new command buffer on a provided device and
  /// manages its resources.
  _CmdBuffer(vuh::Device &device)
    : device(&device)
  {
    auto bufferAI = vk::CommandBufferAllocateInfo(
      device.transferCmdPool(), vk::CommandBufferLevel::ePrimary, 1);
    cmd_buffer = device.allocateCommandBuffers(bufferAI)[0];
  }

  /// Constructor. Takes ownership over the provided buffer.
  /// @pre buffer should belong to the provided device. No check is made even in
  /// a debug build.
...
public: // data
  vk::CommandBuffer
    cmd_buffer; ///< command buffer managed by this wrapper class
  std::unique_ptr<vuh::Device, util::NoopDeleter<vuh::Device>>
    device; ///< device holding the buffer
};          // struct _CmdBuffer
...
struct CopyDevice : private CmdBuffer
{
  ...
  auto copy_async(ArrayIter<Array1> src_begin,
                  ArrayIter<Array1> src_end,
                  ArrayIter<Array2> dst_begin) -> Delayed<>
  {
    assert(device);
    ...
    auto region = vk::BufferCopy(tsize * src_begin.offset(),
                                 tsize * dst_begin.offset(),
...
/// Keeps the staging array and transfer command buffer alive till async copy
/// completes. Delayed action is a noop. At construction copies the data from
/// host to the staging buffer.
template<class T>
struct CopyStageFromHost : public CopyDevice
{
  using StageArray =
    arr::HostArray<T, arr::AllocDevice<arr::properties::HostCoherent>>;
...
/// Keeps the staging buffer and the transfer command buffer alive till async
/// copy completes. Delayed action copies data from staging buffer to the host.
template<class T, class IterDst>
struct CopyStageToHost : CopyDevice
{
  using StageArray =
    arr::HostArray<T, arr::AllocDevice<arr::properties::HostCached>>;
  StageArray array;  ///< staging buffer
  IterDst dst_begin; ///< iterator to beginning of the host destination range

  /// Constructor.
  explicit CopyStageToHost(vuh::Device &device,
                           std::size_t array_size,
                           IterDst dst_begin)
    : CopyDevice(device)
    , array(device, array_size)
    , dst_begin(dst_begin)
  {}
...
/// Wraps movable classes with non-virtual operator()(void) const-> void
/// interface to a class with ICopy virtual interface.
template<class T>
class CopyWrapper
  : public ICopy
  , private T
...
/// Used to trigger some action (encoded in that operator()()) and/or extend
/// resources lifetime at/till the synchrnonization point.
class Copy
{
public:
...
private:
  explicit Copy(std::unique_ptr<detail::ICopy> &&ptr)
    : _obj(std::move(ptr))
  {}
...
template<class Array1, class Array2>
auto
copy_async(ArrayIter<Array1> src_begin,
           ArrayIter<Array1> src_end,
           ArrayIter<Array2> dst_begin) -> vuh::Delayed<Copy>
{
...
/// Blocks while for the duration of initial copy from host memory to
/// host-visible staging array. Only the memory transfer between staging buffer
/// and device memory is actually async. If device array is host-visible the
/// operation is fully blocking.
template<class SrcIter1, class SrcIter2, class T, class Alloc>
auto
copy_async(SrcIter1 src_begin,
           SrcIter2 src_end,
           vuh::ArrayIter<arr::DeviceArray<T, Alloc>> dst_begin)
  -> std::enable_if_t<
    traits::are_comparable_host_iterators<SrcIter1, SrcIter2>::value,
    vuh::Delayed<Copy>>
{
  auto &array = dst_begin.array();
  if (array.isHostVisible()) { // normal copy, the function blocks till the
                               // copying is complete
    array.fromHost(src_begin, src_end, dst_begin.offset());
    return Delayed<Copy>{ array.device(), Copy::wrap(detail::Noop{}) };
  } else { // copy first to staging buffer and then async copy from staging
           // buffer to device
    auto stage =
      detail::CopyStageFromHost<T>(array.device(), src_begin, src_end);
    auto cpy = stage.copy_async(
      device_begin(stage.array), device_end(stage.array), dst_begin);
    return Delayed<Copy>{ std::move(cpy), Copy::wrap(std::move(stage)) };
  }
}

/// Async copy data from device-local array to host.
/// Initiates async copy from device to the staging buffer and immidiately
/// returns the Delayed<Copy>  object used for synchronization with host. The
/// copy between staging buffer and host is only triggered at the
/// synchronization point (Delayed<Copy>::wait() or destructor) and it blocks
/// till the complete operation is finished. If device array is host-visible it
/// just makes the blocking call to std::copy().
template<class T, class Alloc, class DstIter>
auto
copy_async(ArrayIter<arr::DeviceArray<T, Alloc>> src_begin,
                      ...
                      vuh::Delayed<Copy>>
{
  ...
  if (!array.isHostVisible()) { // device array is not host-visible
    auto stage = detail::CopyStageToHost<T, DstIter>(
      array.device(), src_end - src_begin, dst_begin);
    return Delayed<Copy>{ stage.copy_async(
                            src_begin, src_end, device_begin(stage.array)),
    ...
    return Delayed<Copy>{ array.device(),
                          Copy::wrap(detail::StdCopy<SrcIter, DstIter>(
                            src_begin, src_end, dst_begin)) };
  }
}
} // namespace vuh
