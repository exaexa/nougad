...
{
  static constexpr auto descriptor_flags =
...
public:
  ...
  BasicArray(vuh::Device &device ///< device to allocate array
             ...
             ///< additional memory property flags. These are 'added' to flags
             ///< defind by allocator.
             ,
             vk::BufferUsageFlags usage = {}
             ///< additional usage flagsws. These are 'added' to flags defined
             ///< by allocator.
             )
    : vk::Buffer(
        Alloc::makeBuffer(device, size_bytes, descriptor_flags | usage))
    , _dev(device)
  {
    try {
      auto alloc = Alloc();
      ...
      _dev.bindBufferMemory(*this, _mem, 0);
               ...
               &) { // destroy buffer if memory allocation was not successful
      release();
      throw;
    }
  }
  ...
  /// @return offset of the current buffer from the beginning of associated
  /// device memory. For arrays managing their own memory this is always 0.
  ...
  /// @return true if array is host-visible, ie can expose its data via a normal
  /// host pointer.
  ...
  /// Move assignment.
  /// Resources associated with current array are released immidiately (and not
  /// when moved from object goes out of scope).
  auto operator=(BasicArray &&other) noexcept -> BasicArray &
  {
    ...
    _flags = other._flags;
    _dev = other._dev;
    reinterpret_cast<vk::Buffer &>(*this) =
      reinterpret_cast<vk::Buffer &>(other);
...
protected:                        // data
  ...
  vk::MemoryPropertyFlags _flags; ///< actual flags of allocated memory (may
