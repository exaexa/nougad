...
public:
  ...
  auto selectMemory(vk::Buffer buffer, vk::MemoryPropertyFlags properties) const
    -> uint32_t;
...
private: // helpers
  explicit Device(vuh::Instance &instance,
...
private:                    // data
  ...
  vk::CommandPool
    _cmdpool_compute; ///< handle to command pool for compute commands
  vk::CommandBuffer _cmdbuf_compute; ///< primary command buffer associated with
                                     ///< the compute command pool
  vk::CommandPool
    _cmdpool_transfer; ///< handle to command pool for transfer instructions.
                       ///< Initialized on first trasnfer request.
  vk::CommandBuffer
    _cmdbuf_transfer; ///< primary command buffer associated with transfer
                      ///< command pool. Initialized on first transfer request.
  uint32_t _cmp_family_id =
    uint32_t(-1); ///< compute queue family id. -1 if device does not have
                  ///< compute-capable queues.
  uint32_t _tfr_family_id = uint32_t(
    -1); ///< transfer queue family id, maybe the same as compute queue id.
};       // class Device
}
