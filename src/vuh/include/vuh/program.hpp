...
constexpr auto
tuple_element_offset(const T &tup) -> std::size_t
{
...
constexpr auto
spec2entries(const T &specs, std::index_sequence<I...>)
...
template<class... Ts>
auto
typesToDscTypes() -> std::array<vk::DescriptorType, sizeof...(Ts)>
{
...
template<size_t N>
auto
dscTypesToLayout(const std::array<vk::DescriptorType, N> &dsc_types)
{
...
template<class... Ts>
auto
specs2mapentries(const std::tuple<Ts...> &specs)
...
template<class T, size_t... I>
auto
dscinfos2writesets(vk::DescriptorSet dscset,
                   const T &infos,
...
public: // data
  vk::CommandBuffer
    cmd_buffer; ///< command buffer to submit async computation commands
  std::unique_ptr<vuh::Device, util::NoopDeleter<vuh::Device>>
    device; ///< underlying device
};          // struct ComputeData

/// Helper class for use as a Delayed<> parameter extending the lifetime of the
/// command buffer and a noop triggered action.
...
/// Initializes and keeps most state variables, and array argument handling
/// building blocks.
class ProgramBase
{
public:
  /// Run the Program object on previously bound parameters, wait for
  /// completion.
  /// @pre bacth sizes should be specified before calling this.
  /// @pre all paramerters should be specialized, pushed and bound before
  /// calling this.
  auto run() -> void
  {
    auto submitInfo = vk::SubmitInfo(
      ...
      nullptr,
      1,
      &_device.computeCmdBuffer()); // submit a single command buffer
  ...
  auto run_async() -> vuh::Delayed<Compute>
  {
    ...
    auto submitInfo = vk::SubmitInfo(
      0, nullptr, nullptr, 1, &buffer); // submit a single command buffer

    // submit the command buffer to the queue and set up a fence.
    auto queue = _device.computeQueue();
    auto fence = _device.createFence(
      vk::FenceCreateInfo()); // fence makes sure the control is not returned to
                              // CPU till command buffer is depleted
...
protected:
  ...
  ProgramBase(
    vuh::Device &device ///< device used to run the code
    ,
  ...
  /// Construct object using given a vuh::Device a SPIR-V shader code as plain
  /// array.
  ProgramBase(vuh::Device &device ///< device used to run the code
              ,
  ...
  /// Move assignment. Releases resources allocated for current instance before
  /// taking ownership over those of the other instance.
  ...
  auto init_pipelayout(const std::array<vk::PushConstantRange, N> &psrange,
                       Arrs &...) -> void
  {
    auto dscTypes = typesToDscTypes<Arrs...>();
    auto bindings = dscTypesToLayout(dscTypes);
    _dsclayout =
      _device.createDescriptorSetLayout({ vk::DescriptorSetLayoutCreateFlags(),
                                          uint32_t(bindings.size()),
                                          bindings.data() });
    _pipecache = _device.createPipelineCache({});
    _pipelayout =
      _device.createPipelineLayout({ vk::PipelineLayoutCreateFlags(),
                                     1,
                                     &_dsclayout,
                                     uint32_t(N),
                                     psrange.data() });
  ...
  {
    assert(_dsclayout);
    auto sbo_descriptors_size = vk::DescriptorPoolSize(
      vk::DescriptorType::eStorageBuffer, sizeof...(Arrs));
    auto descriptor_sizes = std::array<vk::DescriptorPoolSize, 1>(
      { sbo_descriptors_size }); // can be done compile-time, but not worth it
    _dscpool = _device.createDescriptorPool(
      { vk::DescriptorPoolCreateFlags(),
        1 // 1 here is the max number of descriptor sets that can be allocated
          // from the pool
        ,
        uint32_t(descriptor_sizes.size()),
        descriptor_sizes.data() });
  ...
  auto command_buffer_begin(Arrs &...arrs) -> void
  {
    ...
    auto write_dscsets =
      dscinfos2writesets(_dscset, dscinfos, std::make_index_sequence<N>{});
    _device.updateDescriptorSets(
      write_dscsets, {}); // associate buffers to binding points in bindLayout
    ...
    cmdbuf.bindDescriptorSets(
      vk::PipelineBindPoint::eCompute, _pipelayout, 0, { _dscset }, {});
  }

  /// Ends command buffer creation. Writes dispatch info and signals end of
  /// commands recording.
  auto command_buffer_end() -> void
  {
    auto cmdbuf = _device.computeCmdBuffer();
    cmdbuf.dispatch(_batch[0],
                    _batch[1],
                    _batch[2]); // start compute pipeline, execute the shader
...
protected:                  // data
  ...
  vk::DescriptorSetLayout
    _dsclayout; ///< descriptor set layout. This defines the kernel's array
                ///< parameters interface.
  vk::DescriptorPool
    _dscpool; ///< descitptor ses pool. Descriptors are allocated on this pool.
  ...
  std::array<uint32_t, 3> _batch = {
    0,
    0,
    0
  }; ///< 3D evaluation grid dimensions (number of workgroups to run)
};   // class ProgramBase

/// Part of Program handling specialization constants.
/// Keeps the specialization constants state
/// (between its set by Program::spec() call and actually communicated to
/// device). Does the pipeline init.
...
protected:
  ...
  SpecsBase(Device &device,
            ...
            vk::ShaderModuleCreateFlags f = {})
    : ProgramBase(device, code, f)
  {}

  /// Construct object using given a vuh::Device a SPIR-V shader code in plain
  /// array.
  SpecsBase(Device &device,
            size_t size,
            const uint32_t *code,
  ...
  auto init_pipeline() -> void
  {
    ...
    auto specInfo = vk::SpecializationInfo(uint32_t(specEntries.size()),
                                           specEntries.data(),
                                           sizeof(_specs),
                                           &_specs);

    // Specify the compute shader stage, and it's entry point (main), and
    // specializations
    auto stageCI =
...
protected:
  std::tuple<Spec_Ts...>
    _specs; ///< hold the state of specialization constants between call to
            ///< specs() and actual pipeline creation
};
...
/// Actually runnable entity. Before array parameters are bound (and program
/// run) working grid dimensions should be set up, and if there are
/// specialization constants to set they should be set before that too. Push
/// constants can be specified together with the input/output array parameters
/// right at the calling point. Or alternatively the empty call operator may be
/// triggered on previously fully set up instance of Program.
template<class Specs = typelist<>, class Params = typelist<>>
class Program;
...
public:
  ...
  /// This only sets the dimensions of work batch in units of workgroup, does
  /// not start the actual calculation.
  ...
  /// Initiate execution of the program with provided parameters and immidiately
  /// return.
  /// @return Delayed<Compute> object for synchronization with host.
  /// @pre grid dimensions should be specified before callind this.
  template<class... Arrs>
  auto run_async(const Params &params, Arrs &&...args)
    -> vuh::Delayed<detail::Compute>
  {
    bind(params, args...);
...
private: // helpers
  /// Set up the state of the kernel that depends on number and types of bound
  /// array parameters. Initizalizes the pipeline layout, declares the push
  /// constants interface.
  template<class... Arrs>
  auto init_pipelayout(Arrs &...args) -> void
  {
    auto psranges =
      std::array<vk::PushConstantRange, 1>{ { vk::PushConstantRange(
        vk::ShaderStageFlagBits::eCompute, 0, sizeof(Params)) } };
  ...
  auto create_command_buffer(const Params &p, Arrs &...args) -> void
  {
    ...
    Base::_device.computeCmdBuffer().pushConstants(
      Base::_pipelayout, vk::ShaderStageFlagBits::eCompute, 0, sizeof(p), &p);
    Base::command_buffer_end();
  }
}; // class Program

/// Specialization with non-empty specialization constants and empty push
/// constants.
